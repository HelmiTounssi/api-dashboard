...
data_x_train=app.config['DATA_TRAIN']
data_y_train=app.config['DATA_TEST']
...
def clients():
    """return (test) list of clients"""
    # response = data[['SK_ID_CURR']].to_json()
    response = jsonify(data.reset_index().to_json())
    return response
...
'''
@app.route('/explain/lim',  methods=['GET'])
def explain_lim():
    """
    ...
    Example :
    - http://127.0.0.1:5000/explain/lim/
    """ 
    df_train = pd.read_csv(data_x_train)
    df_test = pd.read_csv(data_y_train)
    target = df_test.drop(columns=['SK_ID_CURR'])
    X_train, X_test, y_train, y_test = train_test_split(df_train.head(1000),target.head(1000), random_state=42)
    model.fit(X_train, y_train)
    # Create the explainer
    explainerlim = lime.lime_tabular.LimeTabularExplainer(X_train, feature_names=X_train.columns, class_names=target.columns)
    # Choose an instance to explain
    instance = X_test[0]
    # Explain the prediction
    explanation = explainerlim.explain_instance(instance, model.predict_proba)
    # Extract feature names and importances
    features = [feature[0] for feature in explanation.as_list()]
    importances = [importance[1] for importance in explanation.as_list()]
    response = jsonify(
        features=features,
